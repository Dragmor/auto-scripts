...
class VKtoTelegram:
    def __init__(self, config):
        try:
            ...
            print(f"Error loading config in vk2tg_poster.py: {error}")
            return
        with open("last_post_date.txt", "r") as file:
    ...
    async def start(self):
        ...
        while True:
            try:
                ...
                print(f"{chat_type}: id={channel_id}")



                print("Starting grab for VK-group...")
                wall = vk.wall.get(owner_id=f"-{group_id}", count=100)
                post_count = wall["count"]
                all_posts.extend(wall["items"])
                temp_posts = []
                print("Last 100 posts grabbing complete!")
                ...
                if all_posts:
                    print(f"Start posts sending ({len(temp_posts)}) to TG")
                else:
                    print("Not new posts in group finded!")
                ...
                for post in reversed(temp_posts):
                    ...
                    try:
                        ...
                        for attachment in attachments:
                            ...
                            # elif attachment
                                ...
                                try:
                                    ...
                                    print("Can't find attachment image!")
                                link_url = attachment["link"]["url"]
                                if link_url not in text:
                                    text += f"\n{link_url}"

                        if not text and media == []:
                            print("Post without content...")
                            continue
                        first_msg = True
                        ...
                        if message_parts == []:
                            while True:
                                try:
                                    ...
                                    print(f"TG antispam system. Wait...")
                        ...
                        # Добавляем задержку
                        print(f"Published: {counter}/{len(temp_posts)} post")
                        counter+=1
                        await asyncio.sleep(self.time_interval)
                    except Exception as error:
                        print(f"Cant send post to TG! {error}\nSkip...")

                

                # записываем последкюю дату поста в файл
                with open("last_post_date.txt", "w") as file:
